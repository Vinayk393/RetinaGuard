...
Running `python src/run_experiment.py --config configs/ce.yaml` with seed=42 and 5-fold TTA should reproduce approximately:
...
| Accuracy | 0.8200 |
| Macro F1 | 0.6528 |
| Referable AUROC | 0.9786 |

Small variation may occur depending on CUDA/cuDNN determinism.

> **Clinical threshold**: At threshold = 0.435, EfficientNet-B0 (CE) achieves **93.3% sensitivity** and **93.3% specificity** for referable DR (grade >= 2), exceeding commonly cited 80% sensitivity and specificity targets for DR screening studies.
...
│   ├── tables/                       # metrics.csv, train_split.csv, val_split.csv, test_split.csv
...
For local execution, update `dataset_root` in the YAML config to the extracted dataset directory on your machine.
...
`notebook/p4 RetinaGuard.ipynb` contains the full EDA, augmentation previews, and inline visualisations. It is for **exploration and figure generation**, not the primary execution path. All experiments should be run via `src/run_experiment.py`.

**To run on Kaggle:**
1. Upload `notebook/p4 RetinaGuard.ipynb` to a new Kaggle notebook
...
- The high QWK coexists with weak Severe-class recall, showing that aggregate metrics can hide minority-class risk.
...
3. **Severe recall** — Grade 3 recall remains a key weakness, with 0.38 recall for the best-QWK CE model, driven by 193 training samples and visual similarity to Grade 2 after Gaussian filtering.
